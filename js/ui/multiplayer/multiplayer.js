...
function hydratePlayerState(player, multiplayerSlot) {
    ...
    hydratedPlayer.hand = hydratedPlayer.hand.map(createVanillaCardState);
    hydratedPlayer.deck = hydratedPlayer.deck.map(createVanillaCardState);
    hydratedPlayer.life = hydratedPlayer.life.map(createVanillaCardState);
    hydratedPlayer.trash = hydratedPlayer.trash.map(createVanillaCardState);
    hydratedPlayer.characters = hydratedPlayer.characters.map(createVanillaCardState);
    hydratedPlayer.leader = createVanillaCardState(hydratedPlayer.leader);
    hydratedPlayer.stage = createVanillaCardState(hydratedPlayer.stage);
    delete hydratedPlayer.skipLeaderRefresh;
    delete hydratedPlayer.loseAtEndOfTurnSource;
...
function restoreBattleUiFromSyncedState() {
    if (pendingAttack) {
        ...
        const leader = document.querySelector(`.board-leader-card[data-player="${defenderKey}"]`);

        if (leader) leader.classList.add("attack-target");

        document.querySelectorAll(`.board-character-card[data-player="${defenderKey}"]`).forEach(element => {
            const slotIndex = Number(element.getAttribute("data-character-slot"));
            const character = gameState[defenderKey]?.characters?.[slotIndex];
    ...
    if (gameState.currentPhase === "counterPhase" || currentAttack.counterPhaseStarted) {
        showCounterPhaseControls(currentAttack.defenderPlayerKey, resolveAndSync);
    } else {
        showResolveAttackButton(currentAttack.defenderPlayerKey, resolveAndSync);
    }
}
...
// =========================
// Effectless Card State
// =========================

function createVanillaCardState(card) {
    if (!card) return card;
    ...
    const vanillaCard = {
        ...
        durationKeywords: [],
        battleKeywords: [],
        temporaryCopiedEffects: [],
        persistentPowerBonus: 0,
        temporaryPowerBonus: 0,
        battlePowerBonus: 0,
        durationPowerBonuses: [],
        costModifiers: []
    };

    delete vanillaCard.cannotAttackUntil;
    delete vanillaCard.cannotAttackThisTurnKey;
    delete vanillaCard.cannotBeRestedUntil;
    delete vanillaCard.effectsNegatedUntil;
    delete vanillaCard.lifeZeroWinCondition;
    delete vanillaCard.protectedFromOpponentEffects;
    delete vanillaCard.skipNextRefresh;
    delete vanillaCard.temporaryBasePower;
...
function updateSidebarControls() {
    ...
    if (sortButton) {
        ...
        sortButton.title = canSort
            ...
            : "Finish the current combat or card-play step before sorting your hand.";
    }
...
function beginAttack(targetData) {
    ...
    currentAttack = {
        ...
        targetPowerBonus: 0,
        resolutionStep: "readyForDefense"
    };
    ...
    addGameLog(`${attackerPlayer.name}'s ${attackerCard.name} attacks ${defenderPlayer.name}'s ${targetCard.name}.`);
...
async function resolveCurrentAttack() {
    ...
    let gameWinner = null;
    const gameOverTitle = "Final Attack";
    const gameOverText = `${defenderPlayer.name} had no life cards left and took a successful leader attack.`;
    ...
    if (attackerWins && currentAttack.target.cardType === "character") {
        const koResult = KOCharacter(defenderPlayer, currentAttack.target.slotIndex, ui, { byBattle: true });
        battleResultText += `<br>${koResult.message}`;
    }

    if (attackerWins && currentAttack.target.cardType === "leader") {
        if (defenderPlayer.life.length === 0) {
            loseByLifeDamage(defenderPlayer, `${defenderPlayer.name} took a direct attack with no life cards remaining.`);
    ...
    if (gameWinner) {
        endGame(gameWinner, gameOverTitle, gameOverText);
        return;
    }
...
function clearCancelAttackButton() {
    document.querySelectorAll(".cancel-attack-button-on-card").forEach(button => button.remove());
}
